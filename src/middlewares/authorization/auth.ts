  ...
  return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!role) {
        ApiResponse(
          false,
          "Un-Authenticated, Role is not available",
          null,
          500,
          res
        );
      ...
      if (!_?.isArray(permission)) {
        ApiResponse(
          false,
          "Un-Authenticated, Role is not provided",
          null,
          500,
          res
        );
      ...
      // Fix for accessing authorization headers
      ...
      return ApiResponse(
        false,
        "Un-Authorize, You are not allowed to access this resource",
        null,
        401,
        res
      );
      ...
      return ApiResponse(
        false,
        "Something Went Wrong, Seems you are not authenticated while accessing",
        error,
        500,
        res
      );
